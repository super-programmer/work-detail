      ...
      <div class="g-container">
        <div class="c-analy-heaher">
          ...
          <!--<div class="c-analy-heaher__operation">
            ...
            <div class="c-analy-heaher__lookdetail">
              <p class="c-analy-heaher__lookdetail&#45;&#45;num font-color&#45;&#45;orange">4<span class="font-size16">次</span>
              </p>
              <p class="c-analy-heaher__lookdetail&#45;&#45;tips">
                观看次数
              </p>
        ...
        <div class="c-res-info__content f-clearfix" v-if="renderData.content">
          <div class="c-res-info__detail">
            ...
            <!-- 资源内容组件 -->
            <viewer :fileId="fileId" :position="position" v-if="fileId" @closeSet="closeSet" @postData="setWorkDetail(10)"></viewer>
          </div>
          <div class="c-res-info__side">
            <div class="c-res-info__profile">
              ...
              <div class="c-res-info__profile__line f-clearfix" v-if="renderData.content.tbkNodes">
                <span class="c-res-info__profile__title">章节：</span>
                <p class="c-res-info__profile__content">{{renderData.content.tbkNodes[0].path.name}}</p>
              </div>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getWorkDetail: function () {
        ...
        axios.get(`https://api.yunguxt.com/work/home/watch/${this.workId}/starting`).then(res => {
          if (res) {
            let data = res.data
            _this.position = data.position
            _this.setOption.timestamp = data.timestamp
            _this.getTaskDetail(data.taskId)
            switch (data.resType) {
              case 3:  //課件
      ...
      getStgSubData: function () {
        this.setData()
        var _this = this
        axios.get(`https://api.yunguxt.com/category/api/subject`).then(res => {
        ...
        axios.get(`https://api.yunguxt.com/category/api/stage`).then(res => {
          res.data.map(function (item) {
